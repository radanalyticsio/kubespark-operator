...
func main(){
	...
	//DeleteCRDCluster(config,"trevor","myproject")
	ListSparkClusters(config,"myproject")
	ScaleSparkCluster(config, "trevor",1,"myproject")
...
func ScaleSparkCluster(config *rest.Config, clusterName string, num int, ns string){
	...
	//if err :=
		sparkCluster, err:=crdclient.Get(clusterName)
		sparkCluster.Spec.Workers= 10
		newSc,_:=crdclient.Update(sparkCluster)
	fmt.Println("SPARKCLUSTER: ",newSc.Spec)

	//	Delete(clusterName,&meta_v1.DeleteOptions{
	//	PropagationPolicy: &deletePolicy,
	//});
	if err != nil {
		panic(err)
	}
	log.Println("Updated crd: ",clusterName," To workers ", num)

}

func ListSparkClusters(config *rest.Config,  ns string){
	fmt.Println("Msg: Listing out objects")
	// List all Example objects
	crdcs, scheme, err := crd.NewClient(config)
	if err != nil {
...
func DeleteCRDCluster( config *rest.Config, clusterName, ns string) {
	crdcs, scheme, err := crd.NewClient(config)
	if err != nil {
		panic(err)
	}
	...
	deletePolicy := meta_v1.DeletePropagationForeground
		...
		if err := crdclient.Delete(clusterName,&meta_v1.DeleteOptions{
			PropagationPolicy: &deletePolicy,
		}); err != nil {
			panic(err)
		}
	log.Println("Deleted crd: ",clusterName)
