...
func (f *crdclient) Create(obj *crd.SparkCluster) (*crd.SparkCluster, error) {
	var result crd.SparkCluster
	err := f.cl.Post().
...
func (f *crdclient) Update(obj *crd.SparkCluster) (*crd.SparkCluster, error) {
	var result crd.SparkCluster
	err := f.cl.Put().
		Namespace(f.ns).Resource(f.plural).
		Body(obj).Do().Into(&result)
...
func (f *crdclient) Get(name string) (*crd.SparkCluster, error) {
	var result crd.SparkCluster
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
...
func (f *crdclient) List(opts meta_v1.ListOptions) (*crd.SparkClusterList, error) {
	var result crd.SparkClusterList
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
